...
fn main() {
    ...
    cmd.arg("-c").arg("echo 'Type stuff. Press Ctrl+B to detach.'; read line; echo \"You typed: $line\"");
    ...
    // Open /dev/tty
    let tty = std::fs::OpenOptions::new().read(true).write(true).open("/dev/tty").unwrap();
    ...
    loop {
        if detached.load(std::sync::atomic::Ordering::SeqCst) { done.store(true, std::sync::atomic::Ordering::SeqCst); break; }
        match child.try_wait() {
            Ok(Some(_)) => { done.store(true, std::sync::atomic::Ordering::SeqCst); break; }
            Ok(None) => std::thread::sleep(std::time::Duration::from_millis(50)),
            Err(_) => { done.store(true, std::sync::atomic::Ordering::SeqCst); break; }
        }
    }
