...
The Wild detection runs in the background every ~2s. clash surfaces every live wild claude PID under the EXTERNAL section. When the process carries `--resume <id>` / `--session-id <id>` in argv (or — rarely — holds the `.jsonl` open as an fd), clash correlates it to the on-disk session and full adoption is offered (`a` → view / takeover / convert). Bare `claude` invocations (no flags) are surfaced as PID-keyed rows so you can see what's running where, but adoption is disabled on those — there's no session id to view, resume, or register. The row disappears on the next scan tick once the process exits, so closed/stopped claudes never linger. List the section in isolation with `:external`.
