        ...
        NavAction::DrillIn { view, context } => {
            ...
            let needs_context = matches!(
                ...
                ViewKind::TeamDetail | ViewKind::AgentDetail | ViewKind::TaskDetail
                    | ViewKind::SessionDetail | ViewKind::SubagentDetail
            );
...
fn reduce_agent(state: &mut AppState, action: AgentAction) -> Vec<Effect> {
    match action {
        ...
        AgentAction::DeleteSession { project, session_id } => {
...
fn reduce_ui(state: &mut AppState, action: UiAction) -> Vec<Effect> {
    match action {
        ...
        UiAction::ToggleAllSessions => {
            ...
            state.table_state.selected = 0;
            let label = if state.show_all_sessions { "Showing all sessions" } else { "Showing running sessions only" };
...
mod tests {
    ...
    fn test_reduce_table_select() {
        ...
        state.store.sessions = vec![
            crate::domain::entities::Session { is_running: true, ..Default::default() },
            crate::domain::entities::Session { is_running: true, ..Default::default() },
            crate::domain::entities::Session { is_running: true, ..Default::default() },
        ];
    ...
    fn test_reduce_toast() {
        ...
        reduce(
            &mut state,
            Action::Ui(UiAction::Toast("hello".to_string())),
        );
    ...
    fn test_reduce_filter_mode() {
        ...
        reduce(
            &mut state,
            Action::Ui(UiAction::SubmitInput(String::new())),
        );
