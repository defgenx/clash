...
impl FsBackend {
    ...
    fn build_session(
        ...
        let display_summary = if !summary.is_empty() {
            ...
            let clean: String = first_prompt.split_whitespace().collect::<Vec<_>>().join(" ");
            if clean.len() > 60 {
                format!("{}...", &clean[..57])
            } else {
                clean
...
impl DataRepository for FsBackend {
    ...
    fn write_task(&self, team: &str, task: &Task) -> Result<()> {
        let path = self.tasks_dir().join(team).join(format!("{}.json", task.id));
                    ...
                    if let Ok(index) = serde_json::from_str::<serde_json::Value>(&index_content) {
                        if let Some(entries) = index.get("entries").and_then(|e| e.as_array()) {
                            for entry in entries {
                                ...
                                let jsonl_path =
                                    project_path.join(format!("{}.jsonl", session_id));
        ...
        for entry in entries {
            ...
            let agent_type = if meta_path.exists() {
                std::fs::read_to_string(&meta_path)
                    ...
                    .and_then(|v| v.get("agentType").and_then(|t| t.as_str()).map(String::from))
                    .unwrap_or_default()
            } else {
                String::new()
            };
    ...
    fn load_conversation(&self, project: &str, session_id: &str) -> Result<Vec<crate::domain::entities::ConversationMessage>> {
    ...
    fn load_subagent_conversation(&self, project: &str, session_id: &str, agent_id: &str) -> Result<Vec<crate::domain::entities::ConversationMessage>> {
        let path = self.base_dir
            .join("projects")
            .join(project)
            .join(session_id)
...
impl FsBackend {
    ...
    fn detect_session_status(jsonl_path: &Path, age_secs: Option<u64>) -> crate::domain::entities::SessionStatus {
        use crate::domain::entities::SessionStatus;
        ...
        for line in tail.lines() {
            ...
            if msg_type == "assistant" {
                ...
                last_assistant_stop_reason = match sr {
                    "end_turn" => { has_end_turn_in_current_turn = true; "end_turn" }
                    "tool_use" => "tool_use",
                    _ => "",
                };
        ...
        if has_end_turn_in_current_turn && (last_type == "system" || last_type == "file-history-snapshot") {
    ...
    fn parse_conversation(path: &Path) -> Result<Vec<crate::domain::entities::ConversationMessage>> {
        use std::io::BufRead;
