...
/// Shimmer gradient keyframes — same palette as the spinner.
...
/// Render a toast notification with shimmer-animated text in the footer area.
pub fn render_toast(message: &str, tick: usize, frame: &mut Frame, area: Rect) {
    let spans: Vec<Span> = message
        .chars()
        .enumerate()
        .map(|(i, ch)| {
