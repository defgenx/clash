...
impl App {
    pub fn new(data_dir: PathBuf, claude_bin: String) -> Self {
        ...
        let watch_paths = vec![backend.teams_dir(), backend.tasks_dir(), backend.projects_dir()];
                ...
                match event {
                    Event::Key(key) => {
                        ...
                        if self.state.input_mode == InputMode::Attached {
                            ...
                            {
                                let action = Action::Ui(crate::application::actions::UiAction::DetachSession);
                    ...
                    Event::Tick => {
                        ...
                        if self.state.tick.is_multiple_of(50)
                            && matches!(self.state.current_view(), crate::adapters::views::ViewKind::Sessions)
                        {
                            self.refresh_daemon_sessions().await;
                        }
        ...
        while let Some(event) = self.daemon.try_recv_event() {
            match event {
                ...
                DaemonEvent::Exited { session_id, .. } => {
                    let action = Action::Ui(crate::application::actions::UiAction::SessionExited { session_id });
    ...
    fn auto_refresh_conversation(&mut self) {
        ...
        match self.state.current_view() {
            ...
            ViewKind::SubagentDetail => {
                if let Some(agent_id) = self.state.nav.current().context.as_deref().map(|s| s.to_string()) {
                    if let Some(sa) = self.state.store.subagents.iter().find(|s| s.id == agent_id).cloned() {
    ...
    async fn execute_effects(&mut self, effects: Vec<Effect>, terminal: &mut ratatui::DefaultTerminal) -> bool {
        ...
        while let Some(effect) = queue.pop_front() {
            match effect {
                ...
                Effect::RefreshSubagents { project, session_id } => {
                    let _ = self.state.store.refresh_subagents(&self.backend, &project, &session_id);
                }
                Effect::LoadConversation { project, session_id } => {
                    let _ = self.state.store.load_conversation(&self.backend, &project, &session_id);
                }
                Effect::LoadSubagentConversation { project, session_id, agent_id } => {
                    let _ = self.state.store.load_subagent_conversation(&self.backend, &project, &session_id, &agent_id);
                }
                Effect::DeleteSession { project, session_id } => {
                ...
                Effect::DaemonCreateSession { session_id, args, cwd } => {
                    ...
                    match self.daemon.create_session(
                        &session_id,
                ...
                Effect::DaemonAttach { session_id } => {
                    ...
                    if !session_id.starts_with("clash-") {
                        ...
                        let _ = self.daemon.create_session(
                            &session_id,
                            &self.cli_runner.claude_bin,
                            &args,
                            "",
                    ...
                    match self.daemon.attach(&session_id).await {
                        Ok(()) => {
                            ...
                            let short = if session_id.len() > 8 { &session_id[..8] } else { &session_id };
                            self.state.toast = Some(format!("Attached to {} | Esc/Ctrl+B detach", short));
                        }
                        Err(e) => {
