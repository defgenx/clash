...
///
/// This is a pure sync loop on fd 0 (stdin) — crossterm must be fully dead
/// (no EventStream, no reader thread) before calling this. The caller is the
/// sole reader on stdin.
///
...
pub async fn run_attach_client(session_id: String) -> eyre::Result<()> {
    ...
    // Enter raw mode for the passthrough loop
