...
impl PtySession {
    ...
    pub fn spawn(
        ...
        for key in &[
            ...
            "LC_CTYPE", "TMPDIR", "XDG_CONFIG_HOME", "XDG_DATA_HOME",
            "SSH_AUTH_SOCK", "ANTHROPIC_API_KEY", "DISPLAY",
        ...
        let replay_buffer: Arc<std::sync::Mutex<Vec<u8>>> = Arc::new(std::sync::Mutex::new(Vec::new()));
        ...
        let screen: Arc<std::sync::Mutex<vt100::Parser>> =
            Arc::new(std::sync::Mutex::new(vt100::Parser::new(screen_rows, screen_cols, 0)));
    ...
    pub fn write_input(&self, data: &[u8]) -> std::io::Result<()> {
        if !self.is_alive() {
            return Err(std::io::Error::new(std::io::ErrorKind::BrokenPipe, "session dead"));
        }
        let n = unsafe {
            libc::write(self.master_fd, data.as_ptr() as *const _, data.len())
        };
        if n < 0 {
    ...
    pub fn replay_data(&self) -> Vec<u8> {
        self.replay_buffer.lock().map(|rb| rb.clone()).unwrap_or_default()
    }
...
fn has_approval_prompt(bottom: &str, last_lines: &[&str]) -> bool {
    ...
    let approval_patterns = [
        ...
        "press y", "press enter to",
        // Claude Code specific tool approval UI
        "run this", "execute", "write to",
