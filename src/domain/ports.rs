...
pub trait DataRepository: Send + Sync {
    ...
    fn load_conversation(&self, project: &str, session_id: &str) -> Result<Vec<ConversationMessage>>;

    /// Load conversation messages from a subagent .jsonl file.
    fn load_subagent_conversation(&self, project: &str, session_id: &str, agent_id: &str) -> Result<Vec<ConversationMessage>>;
