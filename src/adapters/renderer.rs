...
use crate::infrastructure::tui::widgets::{confirm_dialog, detail, help_overlay, input_bar, logo, spinner, table, toast};
...
fn draw_header(state: &AppState, frame: &mut Frame, area: ratatui::layout::Rect) {
    ...
    let waiting_count = state.store.sessions.iter()
        .filter(|s| s.status == SessionStatus::Waiting || s.status == SessionStatus::Prompting)
        .count();

    let mut spans = vec![
        Span::styled(" ✦ clash", Style::default().fg(theme::ACCENT).add_modifier(ratatui::style::Modifier::BOLD)),
    ...
    if waiting_count > 0 {
        spans.push(Span::styled(
            ...
            Style::default().fg(theme::STATUS_WAITING).add_modifier(ratatui::style::Modifier::BOLD),
        ));
    }
...
fn draw_body(state: &AppState, frame: &mut Frame, area: ratatui::layout::Rect, vt_screen: Option<&vt100::Screen>) {
    ...
    match state.current_view() {
        ...
        ViewKind::TeamDetail => detail::render_detail::<team_detail::TeamDetailView>(state, frame, area),
        ViewKind::Agents => table::render_table::<agents::AgentsTable>(state, frame, area),
        ViewKind::AgentDetail => detail::render_detail::<agent_detail::AgentDetailView>(state, frame, area),
        ViewKind::Tasks => table::render_table::<tasks::TasksTable>(state, frame, area),
        ViewKind::TaskDetail => detail::render_detail::<task_detail::TaskDetailView>(state, frame, area),
        ...
        ViewKind::SessionDetail => detail::render_detail::<session_detail::SessionDetailView>(state, frame, area),
        ViewKind::Subagents => table::render_table::<subagents::SubagentsTable>(state, frame, area),
        ViewKind::SubagentDetail => detail::render_detail::<subagent_detail::SubagentDetailView>(state, frame, area),
    }
}
...
fn draw_vt100_terminal(state: &AppState, screen: &vt100::Screen, frame: &mut Frame, area: ratatui::layout::Rect) {
