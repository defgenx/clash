...
- Rendered diff pages are served by the `clashpage://` custom protocol (staged via `stage_browser_page`, FIFO-capped), NOT `data:` URLs — tauri rejects `data:` at webview creation without the `webview-data-url` cargo feature, and WKWebView silently drops multi-MB `data:` URLs (large diffs). The feature stays enabled in gui/src-tauri/Cargo.toml for safety.
