...
- PTY children get `TERM=xterm-256color` + `COLORTERM=truecolor` defaults **only when TERM is missing or `dumb`** (Finder/Dock launches inherit launchd's bare env). A real terminal's values are never overridden, so Claude Code keeps its native colors in both the GUI and the TUI.
