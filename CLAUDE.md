...
- The embedded browser opens rendered diff pages as `data:` URLs — tauri rejects those at webview creation unless the `webview-data-url` cargo feature is enabled (gui/src-tauri/Cargo.toml); without it `browser_open` fails with "invalid window url".
