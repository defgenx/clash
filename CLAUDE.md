...
- The GUI terminals set `macOptionIsMeta: true`, which alone would make brackets/braces untypeable on international Mac layouts (AZERTY types `{`/`[` with Option). A custom key handler in `createTerminal` (gui/dist/app.js) bypasses xterm when Option composes a printable non-letter glyph, so the browser inserts the character while Alt+letter stays Meta.
