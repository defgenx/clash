...
- The GUI terminals set `macOptionIsMeta: true`, which alone would make brackets/braces untypeable on international Mac layouts (AZERTY types `{`/`[` with Option). Merely bypassing xterm (returning false from the custom key handler) does NOT fix it: WKWebView fires no keypress for Option combos and xterm's input-event fallback drops insertText preceded by a keydown (`!e.composed || !this._keyDownSeen`). The handler in `createTerminal` (gui/dist/app.js) must send the composed character to the PTY directly via `send_input`; Alt+letter still goes through xterm as Meta.
